...
namespace Test {
 ...
 "NaN","sNaN"
    ...
    public static string NumberClassString(int nc) {
      // ArgumentAssert.CheckRange(nc,0,9);
      return NumberClasses[nc];
    }

    public static int NumberClass(EDecimal ed, EContext ec) {
      // ArgumentAssert.NotNull(ed);
      if (ed.IsQuietNaN()) return 8;
      if (ed.IsNaN()) return 9;
      if (ed.IsInfinity()) { return ed.IsNegative ? 7 : 6; }
      if (ed.IsZero) { return ed.IsNegative ? 5 : 4; }
      if (IsSubnormal(ed, ec)) {
        return ed.IsNegative ? 3 : 2;
      }
      return ed.IsNegative ? 1 : 0;
    }
    ...
    public static EDecimal LogB(EDecimal ed, EContext ec) {
      ...
      if (ed.IsInfinity()) return EDecimal.PositiveInfinity;
      if (ed.IsZero) {
    ...
    public static EDecimal ScaleB(EDecimal ed, EDecimal ed2, EContext ec) {
      // ArgumentAssert.NotNull(ed);
      // ArgumentAssert.NotNull(ed2);
      if (ed.IsNaN() || ed2.IsNaN()) {
        return ed.Add(ed2, ec);
      }
      if (ed.IsInfinity()) return ed;
      if (!ed2.IsFinite || ed2.Exponent.Sign != 0) return InvalidOperation(EDecimal.NaN, ec);
    ...
    private static EDecimal InvalidOperation(EDecimal ed, EContext ec) {
      if (ec != null) {
        if (ec.HasFlags) ec.Flags |= EContext.FlagInvalid;
    ...
    public static EDecimal And(EDecimal ed1, EDecimal ed2, EContext ec) {
      ...
      if (logi1 == null) return InvalidOperation(ed1, ec);
      byte[] logi2 = FromLogical(ed2);
      if (logi2 == null) return InvalidOperation(ed2, ec);
    ...
    private static EDecimal ToLogical(byte[] bytes) {
      //ArgumentAssert.NotNull(ed);
      //ArgumentAssert.GreaterOrEqual(ed.Sign,0);
    ...
    private static byte[] FromLogical(EDecimal ed) {
      if (!ed.IsFinite || ed.IsNegative || ed.Exponent.Sign != 0 ||
         ed.Mantissa.Sign <= 0) return null;
